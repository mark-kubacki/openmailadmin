	...
	implements INavigationContributor
{
	...
	public function change($mboxnames, $change, $props) {
		...
		if(in_array('mbox', $change)) {
			if($this->oma->imap->renamemb($this->oma->imap->format_user($mboxnames['0']), $this->oma->imap->format_user($props['mbox']))) {
				...
				$this->oma->db->Execute('UPDATE '.$this->oma->tablenames['virtual'].' SET dest=REPLACE(dest, '.$this->oma->db->qstr($mboxnames['0']).', '.$this->oma->db->qstr($props['mbox']).') WHERE dest REGEXP '.$this->oma->db->qstr($mboxnames['0'].'[^@]{1,}').' OR dest LIKE '.$this->oma->db->qstr('%'.$mboxnames['0']));
				$this->oma->db->Execute('UPDATE '.$this->oma->tablenames['virtual_regexp'].' SET dest=REPLACE(dest, '.$this->oma->db->qstr($mboxnames['0']).', '.$this->oma->db->qstr($props['mbox']).') WHERE dest REGEXP '.$this->oma->db->qstr($mboxnames['0'].'[^@]{1,}').' OR dest LIKE '.$this->oma->db->qstr('%'.$mboxnames['0']));
			} else {
	...
	public function delete($mbox_IDs) {
		...
		// virtual
		$this->oma->db->Execute('UPDATE '.$this->oma->tablenames['virtual'].' SET active=0 WHERE '.db_find_in_set($this->oma->db, 'dest', $processed));
		// virtual.regexp
		$this->oma->db->Execute('UPDATE '.$this->oma->tablenames['virtual_regexp'].' SET active=0 WHERE '.db_find_in_set($this->oma->db, 'dest', $processed));
