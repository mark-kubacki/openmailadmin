...
<?php } ?>
<?= caption(txt('79'), getPageList('<a href="'.mkSelfRef(array('mbox_page' => '%d')).'">%d</a>', $oma->current_user->get_number_mailboxes(), $_SESSION['limit'], $_SESSION['offset']['mbox_page']), 580) ?>
			...
			<td class="std">
				...
				<?php } ?>
					<?php if($cfg['mboxview_pers']) { ?>
						<a href="<?= mkSelfRef(array('cuser' => $mailbox['ID'])) ?>" title="<?= $mailbox['mbox'].'; R='.$mailbox['a_super'].':'.$mailbox['a_admin_domains'].':'.$mailbox['a_admin_user'] ?>"><?= $mailbox['person'] ?></a>
					<?php } else { ?>
						<a href="<?= mkSelfRef(array('cuser' => $mailbox['ID'])) ?>" title="<?= $mailbox['person'].'; R='.$mailbox['a_super'].':'.$mailbox['a_admin_domains'].':'.$mailbox['a_admin_user'] ?>"><?= $mailbox['mbox'] ?></a>
					<?php } ?>
					</span>
			</td>
